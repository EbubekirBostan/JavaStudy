...
//Task : "Functional Programming" kullanarak list elemanlarını aynı satirda aralarında bosluk olacak sekilde print ediniz.
    public static void printElFunctional(List<Integer> sayi){
        sayi.//aksiyona girecek list coll.
                ...
                forEach((t)->System.out.print(t+" "));//datanın parametresine göre akışdaki her bir elemanı işleme sokar
    }
    /*
    ...
    */
    public static void printElFunctional1(List<Integer> sayi){//functional pr
        sayi.//aksiyona girecek list coll.
                stream().// List elemanlarını yukarıdan aşağıya akışa alır
                forEach((System.out::print));//Method reference->System.out yapısından print meth call edildi
    ...
    public static void printElFunctional2(List<Integer> sayi){//functional pr
        sayi.//aksiyona girecek list coll.
                stream().// List elemanlarını yukarıdan aşağıya akışa alır
                forEach((C01_LambdaExpression::yazdir));//Method reference->System.out yapısından print meth call edildi
        //MethodKaynagı::Method->meth reference
    }
    public static boolean ciftMi(int sayı){
        return sayı%2==0;
    }
    public static boolean kück35(int sayı){
        return sayı<35;
    }
    public static boolean tekMi(int sayı){
        return sayı%2==1;
    }
    public static int küp1fazlası(int sayı){
        return ((int)Math.pow(sayı,3))+1;
    }
    public static int kareKok(int sayı){
        return ((int)Math.sqrt(sayı));
    }
    public static int karesi(int sayı){
        return ((int)Math.pow(sayı,2));
    }
