...
public class C12_HashTable {
    public static void main(String[] args) {
        ...
        ht.put("çorba","beyran");
        ht.put("söğüş","");
        System.out.println("ht = " + ht);//ht = {köfte=tekirdağ, söğüş=, çorba=beyran, döner=sivas, kebab=adana}
        ht.put("döner","dönerse senindir :)");//döner key update edilir key'de dublicate değil üzerine value ataması yapıkır
        System.out.println("ht = " + ht);//ht = {köfte=tekirdağ, söğüş=, çorba=beyran, döner=dönerse senindir :), kebab=adana}
        //ht.put(null,"trileçe");//RTE NullPointerException
        //ht.put("kahve",null);//RTE NullPointerException
        System.out.println("agam selametle devam et");

        Hashtable<String, String> ht1=new Hashtable<>();
        ht1.put("salata","patlıcan közleme");
        ht1.put("ara sıcak","içli köfte");
        System.out.println("ht1 = " + ht1);//ht1 = {salata=patlıcan közleme, ara sıcak=içli köfte}
        ht.putAll(ht1);
        System.out.println("ht = " + ht);//ht = {köfte=tekirdağ, salata=patlıcan közleme, söğüş=, çorba=beyran, döner=dönerse senindir :), ara sıcak=içli köfte, kebab=adana}
